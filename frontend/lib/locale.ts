export type AppLanguage = "id" | "en";

export const LANGUAGE_STORAGE_KEY = "outurn.language";
export const LANGUAGE_CHANGE_EVENT = "outurn.language.change";

/**
 * Indonesian copy favours warehouse vocabulary people already use. Product names,
 * acronyms, technical protocols, and commonly adopted operational words remain English.
 */
export const terms = {
  id: {
    home: "Beranda",
    build: "Buat",
    overview: "Ringkasan",
    recents: "Terakhir dibuka",
    operations: "Operasional",
    workQueue: "Antrean kerja",
    shipments: "Pengiriman",
    newShipmentCase: "Kasus pengiriman baru",
    documents: "Dokumen",
    parties: "Pihak terkait",
    products: "Produk & komoditas",
    transport: "Transportasi",
    releaseDecisions: "Keputusan pelepasan",
    assurance: "Jaminan proses",
    documentChecks: "Pemeriksaan dokumen",
    requirements: "Persyaratan",
    assuranceChecks: "Pemeriksaan jaminan",
    exceptions: "Pengecualian",
    partyScreening: "Penyaringan pihak",
    dangerousGoods: "Barang berbahaya",
    observe: "Pemantauan",
    analytics: "Analitik",
    observability: "Observability",
    activityLog: "Log aktivitas",
    integrate: "Integrasi",
    connections: "Koneksi",
    webhooks: "Webhooks",
    processingJobs: "Proses latar belakang",
    governance: "Tata kelola",
    rulePacks: "Paket aturan",
    referenceData: "Data referensi",
    manage: "Kelola",
    workspaceSettings: "Pengaturan ruang kerja",
    people: "Pengguna",
    rolesPermissions: "Peran & izin",
    security: "Keamanan",
    notifications: "Notifikasi",
    organizationWorkspace: "Ruang kerja organisasi",
    search: "Cari",
    searchOuturn: "Cari di Outurn",
    searchPlaceholder: "Cari halaman, pengiriman, dokumen...",
    navigate: "Jelajahi Outurn",
    results: "hasil",
    noResults: "Tidak ada data atau halaman yang cocok.",
    close: "Tutup",
    signOut: "Keluar",
    unread: "belum dibaca",
    noNotifications: "Tidak ada notifikasi operasional baru.",
    openSidebar: "Buka sidebar",
    collapseSidebar: "Sembunyikan sidebar",
    systemStatus: "Status sistem",
    documentation: "Dokumentasi",
    privacy: "Privasi",
    searchTitle: "Cari di Outurn",
    closeSearch: "Tutup pencarian",
    escapeToClose: "Esc untuk menutup",
    language: "Bahasa",
    indonesian: "Bahasa Indonesia",
    english: "English",
    checkShipmentDocuments: "Periksa dokumen pengiriman",
    checkShipmentDescription: "Tambahkan tiga dokumen wajib. Outurn membandingkan informasi sebelum shipment dilepas.",
    deliveryOrder: "Surat Jalan",
    packingList: "Daftar kemasan",
    fileTypes: "PDF, JPG, atau PNG",
    documentsReady: "Dokumen siap diperiksa",
    addAllDocuments: "Tambahkan ketiga dokumen untuk melanjutkan",
    uploadCompareReview: "Unggah, baca, bandingkan, lalu tinjau hasilnya.",
    checkingDocuments: "Memeriksa dokumen...",
    checkDocuments: "Periksa dokumen",
    readingDocuments: "Dokumen sedang dibaca dan dibandingkan. Hasil akan tampil setelah pemeriksaan selesai.",
    loginTitle: "Masuk ke Outurn",
    loginIntro: "Periksa dokumen pengiriman, selesaikan pengecualian, dan catat keputusan pelepasan.",
    email: "Email",
    password: "Password",
    showPassword: "Tampilkan password",
    hidePassword: "Sembunyikan password",
    rememberEmail: "Simpan email di perangkat ini",
    signIn: "Masuk",
    signingIn: "Memeriksa akses…",
    needAccess: "Butuh akses? Hubungi administrator Outurn.",
    loginLegal: "Dengan melanjutkan, Anda menyetujui ketentuan dan kebijakan privasi Outurn.",
    evidenceWorkspace: "Workspace pemeriksaan pengiriman",
    loginPanelEyebrow: "Pemeriksaan sebelum pelepasan",
    loginPanelTitle: "Setiap keputusan pelepasan didukung bukti.",
    loginPanelBody: "Satukan dokumen, temuan, dan persetujuan dalam satu catatan operasional yang jelas.",
  },
  en: {
    home: "Home",
    build: "Build",
    overview: "Overview",
    recents: "Recents",
    operations: "Operations",
    workQueue: "Work queue",
    shipments: "Shipments",
    newShipmentCase: "New shipment case",
    documents: "Documents",
    parties: "Parties",
    products: "Products & commodities",
    transport: "Transport",
    releaseDecisions: "Release decisions",
    assurance: "Assurance",
    documentChecks: "Document checks",
    requirements: "Requirements",
    assuranceChecks: "Assurance checks",
    exceptions: "Exceptions",
    partyScreening: "Party screening",
    dangerousGoods: "Dangerous goods",
    observe: "Observe",
    analytics: "Analytics",
    observability: "Observability",
    activityLog: "Activity log",
    integrate: "Integrate",
    connections: "Connections",
    webhooks: "Webhooks",
    processingJobs: "Processing jobs",
    governance: "Governance",
    rulePacks: "Rule packs",
    referenceData: "Reference data",
    manage: "Manage",
    workspaceSettings: "Workspace settings",
    people: "People",
    rolesPermissions: "Roles & permissions",
    security: "Security",
    notifications: "Notifications",
    organizationWorkspace: "Organization workspace",
    search: "Search",
    searchOuturn: "Search Outurn",
    searchPlaceholder: "Search pages, shipments, documents...",
    navigate: "Navigate Outurn",
    results: "results",
    noResults: "No matching workspace records or pages.",
    close: "Close",
    signOut: "Sign out",
    unread: "unread",
    noNotifications: "No new operational notifications.",
    openSidebar: "Open sidebar",
    collapseSidebar: "Collapse sidebar",
    systemStatus: "System status",
    documentation: "Documentation",
    privacy: "Privacy",
    searchTitle: "Search Outurn",
    closeSearch: "Close search",
    escapeToClose: "Esc to close",
    language: "Language",
    indonesian: "Bahasa Indonesia",
    english: "English",
    checkShipmentDocuments: "Check shipment documents",
    checkShipmentDescription: "Add the three required documents. Outurn compares the information before release.",
    deliveryOrder: "Delivery order",
    packingList: "Packing list",
    fileTypes: "PDF, JPG, or PNG",
    documentsReady: "Documents are ready",
    addAllDocuments: "Add all three documents to continue",
    uploadCompareReview: "Upload, read, compare, then review.",
    checkingDocuments: "Checking documents...",
    checkDocuments: "Check documents",
    readingDocuments: "We are reading and comparing the documents. The result will appear when the check is complete.",
    loginTitle: "Sign in to Outurn",
    loginIntro: "Review shipment documents, resolve exceptions, and record release decisions.",
    email: "Email",
    password: "Password",
    showPassword: "Show password",
    hidePassword: "Hide password",
    rememberEmail: "Save email on this device",
    signIn: "Sign in",
    signingIn: "Checking access…",
    needAccess: "Need access? Contact your Outurn administrator.",
    loginLegal: "By continuing, you agree to Outurn's terms and privacy policy.",
    evidenceWorkspace: "Shipment assurance workspace",
    loginPanelEyebrow: "Pre-release assurance",
    loginPanelTitle: "Every release decision is backed by evidence.",
    loginPanelBody: "Bring documents, findings, and approvals into one clear operational record.",
  },
} as const;

export type LocaleKey = keyof typeof terms.id;

export function languageSnapshot(): AppLanguage {
  if (typeof window === "undefined") return "id";
  return window.localStorage.getItem(LANGUAGE_STORAGE_KEY) === "en" ? "en" : "id";
}

export function setLanguage(language: AppLanguage) {
  window.localStorage.setItem(LANGUAGE_STORAGE_KEY, language);
  window.dispatchEvent(new Event(LANGUAGE_CHANGE_EVENT));
}

export function subscribeToLanguage(callback: () => void) {
  window.addEventListener(LANGUAGE_CHANGE_EVENT, callback);
  return () => window.removeEventListener(LANGUAGE_CHANGE_EVENT, callback);
}

export function translate(language: AppLanguage, key: LocaleKey) {
  return terms[language][key];
}
