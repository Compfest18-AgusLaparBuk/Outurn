...
export default function ShipmentsPage() {
  ...
  const result = useQuery({
    queryKey: ["shipments", params.toString()],
    queryFn: () => fetchShipments(params),
  });

  return (
    <CloudflarePageShell className="operations-page shipments-page">
      <PageHeader
        icon={Package}
        title="Pengiriman"
        description="Buat kasus pengiriman, bandingkan dokumennya, dan kelola keputusan pelepasan dari satu tempat."
        actions={
          <ActionLink href="/shipments/new" icon={Plus}>
            Buat pengiriman
          </ActionLink>
        }
      />
      <FilterBar className="shipments-toolbar" label="Filter pengiriman">
        <SearchField
          ...
          placeholder="Cari referensi atau tujuan pengiriman"
          ariaLabel="Cari pengiriman"
        />
        {result.data && (
          <span className="cf-metadata">{result.data.total} hasil</span>
        )}
        <AppSelect
      ...
      {result.isPending ? (
        <LoadingState label="Memuat pengiriman…" className="page-loading" />
      ) : result.isError ? (
        <StateNotice
          title="Daftar pengiriman tidak tersedia saat ini."
          tone="danger"
        >
        ...
        <DataTableSurface
          ...
          description={`${result.data.total} kasus pengiriman di ruang kerja ini.`}
        >
          {result.data.items.length === 0 ? (
            <EmptyState
              icon={<Package size={22} />}
              title="Belum ada pengiriman"
              description="Buat kasus pengiriman pertama untuk memulai pemeriksaan dokumen."
              action={
                <ActionLink href="/shipments/new">Buat pengiriman</ActionLink>
              }
            />
          ) : (
              ...
              <Table>
                <Table.Header sticky>
                  <Table.Row>
                    ...
                    <Table.Head>Pemeriksaan terbuka</Table.Head>
                      ...
                      <Table.Cell>
                        ...
                        <small>
                          {shipment.external_reference ||
                            "Tidak ada referensi eksternal"}
                        </small>
                      </Table.Cell>
                      <Table.Cell>
                        {shipment.origin} <span aria-hidden="true">→</span>{" "}
                        {shipment.destination}
                      </Table.Cell>
                      <Table.Cell>{shipment.transport_mode}</Table.Cell>
                      <Table.Cell>
                        <ShipmentState status={shipment.status} />
                        <small>
                          Risiko {shipment.risk_level.toLowerCase()}
                        </small>
                      </Table.Cell>
                      <Table.Cell>{shipment.open_tasks}</Table.Cell>
                      <Table.Cell>
                        {new Date(shipment.updated_at).toLocaleString("id-ID")}
                      </Table.Cell>
                      <Table.Cell>
                        <ActionLink
                          href={`/shipments/${shipment.id}`}
