...
export default function WorkQueuePage() {
  ...
  const result = useQuery({
    ...
    queryFn: () => fetchWorkQueue(new URLSearchParams(query)),
  });
  const now = result.dataUpdatedAt || null;
  const update = useMutation({
    mutationFn: (value: { id: string; status: "IN_PROGRESS" | "RESOLVED" }) =>
      updateWorkQueue(value.id, value.status),
    onSuccess: () => client.invalidateQueries({ queryKey: ["work-queue"] }),
  });
  const items = useMemo(
    () =>
      (result.data?.items || []).filter((task) => {
        if (!due || !task.due_at)
          return due !== "overdue" || Boolean(task.due_at);
  ...
  return (
    <CloudflarePageShell className="cf-work-queue-page">
      <PageHeader
        icon={ListChecks}
        title="Antrean kerja"
        description="Ambil dan selesaikan pemeriksaan yang memerlukan tindakan manusia sebelum pengiriman bergerak."
        actions={
          <ActionLink href="/shipments" variant="secondary">
            Cari pengiriman
          </ActionLink>
        }
      />
      <FilterBar className="cf-work-queue-toolbar" label="Filter antrean kerja">
        <AppSelect
          ariaLabel="Filter status antrean"
          value={status}
          onValueChange={(value) => {
            setPage(1);
            setStatus(value);
        ...
        />
        <AppSelect
          ariaLabel="Filter prioritas antrean"
          value={priority}
          onValueChange={(value) => {
            setPage(1);
            setPriority(value);
        ...
        />
        {result.data && (
          <span className="cf-metadata">{result.data.total} tugas</span>
        )}
      </FilterBar>
      {result.isPending ? (
        <LoadingState label="Memuat antrean kerja…" className="page-loading" />
      ) : result.isError ? (
        <StateNotice
          title="Antrean kerja tidak tersedia saat ini."
          tone="danger"
        >
        ...
        <>
          {due === "overdue" && (
            <StateNotice tone="warning" title="Filter terlambat aktif">
              Tugas yang sudah melewati jatuh tempo.
            </StateNotice>
          )}
          <DataTableSurface
            title="Tugas operasional"
            description={`${items.length} tugas cocok dengan filter aktif.`}
          >
            {items.length === 0 ? (
              <EmptyState
                icon={<ListChecks size={20} />}
                title="Tidak ada tugas pada tampilan ini"
                description="Ubah filter atau kembali setelah penilaian membuat tugas baru."
              />
            ) : (
                  ...
                  <Table.Body>
                    {items.map((task) => (
                      <Table.Row key={task.id}>
                        <Table.Cell>
                          <OperationalState value={task.priority} />
                        </Table.Cell>
                        <Table.Cell>
                        ...
                        <Table.Cell>
                          <Link
                            ...
                            href={`/shipments/${task.shipment_id}`}
                          >
                            {task.shipment_reference}
                          </Link>
                        </Table.Cell>
                        ...
                        <Table.Cell>
                          <OperationalState value={task.status} />
                        </Table.Cell>
                        <Table.Cell>
                          {task.status === "OPEN" ? (
                            <Button
          ...
          {items.some((task) => isOverdue(task.due_at, now)) && (
            <p className="cf-work-queue-note">
              <WarningCircle size={15} /> Jatuh tempo menggunakan waktu yang
              tersimpan pada tugas.
            </p>
          )}
        </>
      )}
