...
> = {
  documents: {
    title: "Documents",
    description:
      "Keep shipment evidence versioned, reviewable, and connected to its case.",
    path: "/documents",
    columns: [
      {
        label: "Document",
      ...
      { label: "Shipment", value: (row) => String(row.shipment_reference) },
      {
        ...
        value: (row) =>
          row.version
            ? `v${String((row.version as Record<string, unknown>).version)}`
            : "—",
      },
      {
        label: "Extraction",
  ...
  parties: {
    title: "Parties",
    description: "Maintain the trading parties involved in shipment cases.",
    path: "/parties",
    columns: [
      {
        label: "Party",
  ...
  products: {
    ...
    description:
      "See the items moving through your shipment cases and the evidence attached to them.",
    path: "/products",
    columns: [
      {
        label: "SKU",
      ...
      { label: "Shipment", value: (row) => String(row.shipment_reference) },
      {
        label: "Quantity",
        value: (row) =>
          `${String(row.quantity)} ${String(row.unit_of_measure)}`,
      },
      {
        label: "Dangerous goods",
        value: (row) => (row.dangerous_goods ? "Review required" : "No"),
      },
    ],
  },
  transport: {
    title: "Transport",
    description:
      "Record planned movement, carriers, and equipment without pretending to provide live tracking.",
    path: "/transport",
    columns: [
      {
        label: "Mode",
      ...
      { label: "Destination", value: (row) => String(row.destination || "—") },
      {
        ...
        value: (row) =>
          row.planned_arrival
            ? new Date(String(row.planned_arrival)).toLocaleString("en-GB")
            : "—",
      },
    ],
  },
  requirements: {
    title: "Requirements",
    description:
      "Understand which evidence is expected for each shipment and why.",
    path: "/requirements",
    columns: [
      {
        label: "Requirement",
        value: (row) => (
          <span className="table-cell-primary">
            {String((row.requirement as Record<string, unknown>).name)}
          </span>
        ),
      },
      { label: "Shipment", value: (row) => String(row.shipment_reference) },
      ...
      },
      {
        ...
        value: (row) => (
          <StatusValue
            value={(row.evaluation as Record<string, unknown>).result}
          />
        ),
      },
      {
        label: "Reason",
        value: (row) =>
          String((row.evaluation as Record<string, unknown>).reason),
      },
    ],
  },
  assurance: {
    ...
    description:
      "Review checks across shipment cases, with evidence and the rule version that produced them.",
    path: "/assurance",
    columns: [
      {
        label: "Shipment",
  ...
  exceptions: {
    title: "Exceptions",
    description: "Resolve the issues that keep a shipment from moving forward.",
    path: "/exceptions",
    columns: [
      {
        label: "Severity",
        value: (row) => <StatusValue value={row.severity} />,
      },
      {
        label: "Shipment",
        value: (row) => (
      ...
      {
        label: "Assignee",
        value: (row) => String(row.assigned_to || "Unassigned"),
      },
      {
        label: "Due",
        value: (row) =>
  ...
  releases: {
    ...
    description:
      "Review the immutable decisions that authorize, hold, or invalidate dispatch.",
    path: "/releases",
    columns: [
      {
        label: "Shipment",
  ...
  screening: {
    ...
    description:
      "Keep screening honest: configured providers report results; unconfigured providers do not claim coverage.",
    path: "/screening",
    columns: [
      {
        label: "Party",
        value: (row) => (
          <span className="table-cell-primary">{String(row.party)}</span>
        ),
      },
      { label: "Shipment", value: (row) => String(row.shipment_reference) },
      {
      ...
      },
      {
        label: "Score",
        value: (row) =>
          String((row.run as Record<string, unknown>).score ?? "—"),
      },
    ],
  },
  "dangerous-goods": {
    ...
    description:
      "Review items that need complete dangerous-goods information before release.",
    ...
    columns: [
      ...
      },
      {
        label: "Item",
        value: (row) =>
          String((row.item as Record<string, unknown>).description),
      },
      {
        label: "UN number",
        value: (row) =>
          String((row.item as Record<string, unknown>).un_number || "Missing"),
      },
      {
        label: "Hazard class",
        value: (row) =>
  ...
  connections: {
    title: "Connections",
    description:
      "See the systems configured to exchange shipment information with this workspace.",
    ...
    columns: [
      ...
      { label: "Status", value: (row) => operationalStatusLabel(String(row.status)) },
      {
        ...
        value: (row) =>
          row.last_success_at
            ? new Date(String(row.last_success_at)).toLocaleString("en-GB")
            : "—",
      },
      {
        label: "Last error",
        value: (row) =>
          row.last_error_at
            ? new Date(String(row.last_error_at)).toLocaleString("en-GB")
            : "—",
      },
    ],
  },
  jobs: {
    ...
    description:
      "Track bounded extraction, assessment, and delivery work without hiding failures.",
    ...
    columns: [
      ...
      {
        label: "Queued",
        value: (row) => new Date(String(row.queued_at)).toLocaleString("en-GB"),
      },
...
export function OperationRegister({
  ...
  const result = useQuery({
    queryKey: ["operations", kind, query],
    queryFn: () =>
      fetchOperationsList(config.path, query ? { q: query } : undefined),
  });
  const rows = useMemo(() => result.data?.items || [], [result.data]);
  return (
    <CloudflarePageShell className="operations-page">
      {includeHeader && (
        <PageHeader
          title={config.title}
      ...
      >
        <div className="operations-search">
          ...
          />
          <span className="cf-metadata">{rows.length} records</span>
        </div>
      </FilterBar>
      {result.isError ? (
        <StateNotice title="Register tidak tersedia saat ini." tone="danger">
        ...
        <DataTableSurface>
          ...
          {!result.isPending && rows.length === 0 && (
            <EmptyState
              ...
              description="Records appear here when your team creates them in this workspace."
            />
          )}
        </DataTableSurface>
      )}
