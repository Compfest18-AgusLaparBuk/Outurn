...
import { fetchWorkspaceContext, overrideDecision } from "@/lib/api";
import type {
  DocumentType,
  Mismatch,
...
export function ResultWorkspace({
  ...
  const [selected, setSelected] = useState<Mismatch | null>(
  ...
  const effectiveStatus =
    result.audit.final_decision || result.effective_status || result.status;
  ...
  return (
    <div className="cf-reconciliation-workspace">
        ...
        <div className="cf-reconciliation-header__inner">
          <div className="cf-reconciliation-header__summary">
            ...
            <div className="min-w-0">
              <p className="cf-reconciliation-header__reason">
                {result.reason}
              </p>
              <p className="cf-metadata">
                {result.mismatches.length} isu · {result.processing_ms} ms ·{" "}
                <span className="mono">{result.session_id.slice(0, 8)}</span>
              </p>
            </div>
          </div>
          <div className="cf-reconciliation-header__actions">
            ...
            {(context?.role === "supervisor" || context?.role === "admin") && (
              <Button
                variant={effectiveStatus === "HOLD" ? "danger" : "primary"}
      ...
      <main className="cf-reconciliation-grid">
        ...
        <aside className="cf-reconciliation-aside">
          <DataTableSurface
            ...
            actions={
              <span className="cf-metadata">
                {result.mismatches.length} temuan
              </span>
            }
          >
            {result.mismatches.length === 0 ? (
              <EmptyState
                ...
                title="Tidak ada konflik material"
                description="Tidak ada konflik material yang terdeteksi pada dokumen ini."
              />
            ) : (
              <div className="cf-finding-list">
                {result.mismatches.map((mismatch) => (
                  <Button
                    ...
                    className={`cf-finding-list__item ${selected?.id === mismatch.id ? "cf-finding-list__item--selected" : ""}`}
                  >
                    <span className="cf-finding-list__copy">
                      <span className="cf-finding-list__heading">
                        <span>{mismatch.type.replaceAll("_", " ")}</span>
                        <Badge
                          appearance="dot"
                          variant={severityVariant(mismatch.severity)}
                        >
                          {operationalStatusLabel(mismatch.severity)}
                        </Badge>
                      </span>
                      <span className="cf-finding-list__description">
          ...
          {selected && (
            <DataTableSurface
              className="cf-evidence-surface"
              title="Perbandingan bukti"
            >
                ...
                <Table>
                  <Table.Header sticky>
                    <Table.Row>
                      ...
                      <Table.Head>Tingkat keyakinan</Table.Head>
                    </Table.Row>
                  </Table.Header>
                  <Table.Body>
                    {selected.evidence.map((item, index) => (
                      <Table.Row key={`${item.document_type}-${index}`}>
                        <Table.Cell>{docLabels[item.document_type]}</Table.Cell>
                        <Table.Cell>
                          <span className="table-cell-primary break-words">
                            {String(item.value ?? "—")}
                          </span>
                        </Table.Cell>
                        <Table.Cell>
              ...
              {selected.estimated_discrepancy_value != null && (
                <div className="cf-evidence-surface__estimate">
                  Estimasi nilai selisih:{" "}
                  <span className="table-cell-primary">
                    Rp{" "}
                    {selected.estimated_discrepancy_value.toLocaleString(
                      "id-ID",
                    )}
                  </span>
                  {selected.estimate_price_source
                    ? ` · harga dari ${docLabels[selected.estimate_price_source]}`
                    : ""}
                </div>
              )}
          ...
          {result.audit.final_decision && (
            <StateNotice
              title={`Override tercatat: ${result.audit.final_decision}`}
              tone="info"
            >
              <span>
                {result.audit.override_reason}
                {result.audit.overridden_by
                  ? ` · Supervisor: ${result.audit.overridden_by}`
                  : ""}{" "}
                · Keputusan sistem asli tetap: {result.audit.system_decision} ·{" "}
                {result.audit.override_history.length} peristiwa audit
              </span>
            </StateNotice>
          )}
        </aside>
...
function OverrideDialog({
  ...
  const [decision, setDecision] =
    useState<ReconciliationStatus>(systemDecision);
  const [reason, setReason] = useState("");
  const mutation = useMutation({
    mutationFn: () =>
      overrideDecision(sessionId, { final_decision: decision, reason }),
    onSuccess: (data) => {
      ...
      <Dialog className="override-dialog" size="base">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <Dialog.Description>
              Keputusan sistem asli akan tetap disimpan untuk audit.
            </Dialog.Description>
          </div>
        </div>
        <p className="override-dialog__notice">
